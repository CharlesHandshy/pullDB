...
# =============================================================================
# Phase 3 — Snapshot (mysqldump from active container)
# =============================================================================
phase3_snapshot() {
    log_step "3 — Snapshot"

    DUMP_FILE="${UPGRADE_TMP}/pulldb-${ACTIVE_COLOR}-$(date +%Y%m%dT%H%M%S).sql"

    if [[ "$DRY_RUN" == true ]]; then
        log_dry "Would mkdir -p ${UPGRADE_TMP}"
        log_dry "Would mysqldump pulldb_service → ${DUMP_FILE}"
        DUMP_FILE="${UPGRADE_TMP}/pulldb-snapshot.sql"  # placeholder for dry run
        return
    fi
    ...
    log_info "Dumping pulldb_service from ${ACTIVE_CONTAINER}..."
    docker exec "$ACTIVE_CONTAINER" \
        mysqldump \
            --single-transaction \
            --routines \
    ...
    local size
    size=$(du -sh "$DUMP_FILE" 2>/dev/null | cut -f1 || echo "?")
    log_info "Snapshot: ${DUMP_FILE} (${size})"
}
...
phase4_candidate() {
    ...
    if [[ "$DRY_RUN" == true ]]; then
        ...
        log_dry "  PULLDB_IMPORT_DUMP=/tmp/pulldb-upgrade/$(basename "${DUMP_FILE}")"
        return
    fi
...
PULLDB_IMPORT_DUMP=/tmp/pulldb-upgrade/$(basename "${DUMP_FILE}")
EOF
    ...
    log_info "(Container is initializing — validate.sh will poll until ready)"
}
...
phase6_promote() {
    ...
    # --- Pause active container (keep for rollback) ---
    log_info "Pausing active container: ${ACTIVE_CONTAINER}"
    docker compose \
    ...
    # --- Write promoted env (real ports, no import dump) ---
