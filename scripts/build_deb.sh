...
if [ -d "pulldb/binaries" ]; then
    ...
    # Copy unified myloader binary (0.21.1-1 supports all backup formats via metadata synthesis)
    if [ -f "pulldb/binaries/myloader-0.21.1-1" ]; then
        cp pulldb/binaries/myloader-0.21.1-1 "$APP_ROOT/bin/"
        chmod +x "$APP_ROOT/bin/myloader-0.21.1-1"
        echo "Included myloader-0.21.1-1 binary"
    else
        echo "[ERROR] myloader-0.21.1-1 binary not found in pulldb/binaries/" >&2
        exit 1
    fi
fi
