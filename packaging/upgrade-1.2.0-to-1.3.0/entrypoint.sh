    ...
    for subdir in 00_tables 01_views 02_seed 03_users; do
        if [[ -d "${SCHEMA_DIR}/${subdir}" ]]; then
            for sql_file in "${SCHEMA_DIR}/${subdir}"/*.sql; do
                ...
                # Use if/else to capture both stdout+stderr and exit code under set -e.
                # A bare `sql_err=$(mysql ...); sql_exit=$?` exits the script with set -e
                # before the exit-code line is reached when mysql returns non-zero.
                if sql_err=$(mysql pulldb_service < "$sql_file" 2>&1); then
                    sql_exit=0
                else
                    sql_exit=$?
                fi
