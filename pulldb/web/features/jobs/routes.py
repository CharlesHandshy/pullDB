...
from pulldb.domain.models import User, JobStatus, UserRole
...
async def jobs_page(
    ...
    if hasattr(state, "job_repo") and state.job_repo:
        if view == "history":
            ...
            if hasattr(state.job_repo, "get_job_history"):
                ...
                if status:
                    jobs = [j for j in jobs if str(j.status).lower() == status.lower() or j.status.value == status]
                if host:
                    jobs = [j for j in jobs if j.dbhost == host]
                if user_code:
                    jobs = [j for j in jobs if getattr(j, 'owner_user_code', None) == user_code]
        else:
            # Active view
            if q and len(q) >= 4:
                # Search mode
    ...
    return templates.TemplateResponse(
        "features/jobs/jobs.html",
        {
            ...
            "three_days_ago_iso": (datetime.now(timezone.utc) - timedelta(days=3)).isoformat(),
        },
    )
...
async def job_details(
    ...
    if hasattr(state, "job_repo") and state.job_repo:
        ...
        if job:
            ...
            job_can_cancel = can_cancel_job(user, job.owner_user_id, job_owner_manager_id)
