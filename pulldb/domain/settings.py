...
SETTING_REGISTRY: dict[str, SettingMeta] = {
    ...
    "workdir_cleanup_interval": SettingMeta(
        key="workdir_cleanup_interval",
        env_var="PULLDB_WORKDIR_CLEANUP_INTERVAL",
        default="3600",
        description="Seconds between orphan work directory scans (0 to disable)",
        setting_type=SettingType.INTEGER,
        category=SettingCategory.CLEANUP,
        dangerous=False,
        validators=["is_non_negative_integer"],
    ),
    "workdir_cleanup_dry_run": SettingMeta(
        key="workdir_cleanup_dry_run",
        env_var="PULLDB_WORKDIR_CLEANUP_DRY_RUN",
        default="true",
        description="Dry-run mode: log orphan dirs but don't delete (safe rollout)",
